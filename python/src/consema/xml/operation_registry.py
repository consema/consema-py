...
- RFC 0012 §11 (https://github.com/consema/consema/blob/main/docs/rfcs/0012-xml-1.0-safe-profile-v1.md) freezes
...
- The exact descriptors transcribe https://github.com/consema/consema-rs/blob/main/consema-xml/src/
  operation_registry.rs (ids, target roles xml.text@1 /
  ...
  operation_registry.rs pins the frozen eight-operation surface.
...
def format_operation_registry(profile_id: ProfileId) -> FormatOperationRegistry:
    ...
    (operation_registry.rs); the profile must be ``xml.1.0-safe@1``."""
...
# operation_registry.rs (the surface test at 100-124 pins the
# sorted id list).
