...
- Targets, comparison, and collision policy: https://github.com/consema/consema-rs/blob/main/consema-ini/src/
  projection.rs — ProjectionTarget::BestExactEntryMappingV1 /
  RequireObjectV1 (projection.rs), NameComparison OriginalExact /
  ProfileEquivalent (projection.rs), CollisionPolicy Reject / First /
  Last (projection.rs); the request builders projection.rs;
...
- Gates and failure algebra: projection.rs — Recovered documents
  ...
  projection.rs and the failed-attempt arguments projection.rs
 (reason, limit, profile); failed attempts never contain a
  partial value (RFC 0004 §7, https://github.com/consema/consema/blob/main/docs/rfcs/0004-materialization-conversion-and-structural-edit-v1.md).
- Exact mapping: projection.rs — every section occurrence maps to
...
- Object collapse: projection.rs — explicit comparison and
...
- The Python default section is an ordinary association whose provenance
  ...
  (RFC 0009 §10, https://github.com/consema/consema/blob/main/docs/rfcs/0009-ini-family-profiles-v1.md).
...
class ProjectionTarget(enum.Enum):
    ...
    ``RequireObjectV1`` (RFC 0009 §10, https://github.com/consema/consema/blob/main/docs/rfcs/0009-ini-family-profiles-v1.md).
    """
...
class Fidelity(enum.Enum):
    ...
    def lifted(self, other: Fidelity) -> Fidelity:
        """Worst-of fidelity (projection.rs)."""
...
class FailedProjectionAttempt:
    """Failed attempt with no value or provenance map (projection.rs)."""
...
class _Context:
    ...
    def add_entry_value_origins(
        ...
        (projection.rs; RFC 0009 §10, https://github.com/consema/consema/blob/main/docs/rfcs/0009-ini-family-profiles-v1.md)."""
...
def _select_indices(
    ...
    """Explicit collision selection (projection.rs)."""
    counts: dict[str, int] = {}
    for name in names:
...
def _group_entries(document: IniDocument) -> dict[NodeRef, tuple[int, ...]]:
    """Entry ordinals grouped by owning section node (projection.rs)."""
...
def _comparison_name(
    ...
    """Profile comparison name under the request mode (projection.rs)."""
...
def _failed(
    ...
    *,
    ...
    """Failed attempt with one stable diagnostic (projection.rs)."""
