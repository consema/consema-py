...
- ``JsonProfile``: the three profile identities and their extension surface —
  https://github.com/consema/consema-rs/blob/main/consema-json/src/lib.rs (enum), lib.rs (id() and
  ...
  spellings (RFC 0005 §1, https://github.com/consema/consema/blob/main/docs/rfcs/0005-json-family-production-v1.md;
...
- ``JsonSyntaxKind``: the closed 17-kind lossless classification with the
  ...
  "ErrorRegion") — https://github.com/consema/consema-rs/blob/main/consema-json/src/lib.rs (enum + as_str),
  lib.rs (from_name). ``Identifier`` is a v2-domain kind added by
...
- ``JsonValueKind``: the native semantic categories (Null, Boolean,
  Integer, Decimal, BinaryFloat64, String, Array, Object) — lib.rs and
  the kind mapping lib.rs; BinaryFloat64 is the JSON5 extension
  (RFC 0005 §6, https://github.com/consema/consema/blob/main/docs/rfcs/0005-json-family-production-v1.md).
- ``SemanticUnavailable`` reasons (Missing, InvalidLiteral, ErrorRegion) —
  lib.rs; ``SemanticAvailability`` mirror of lib.rs.
- JSON5 whitespace / line-terminator / IdentifierName character rules —
  RFC 0005 §3/§4 (https://github.com/consema/consema/blob/main/docs/rfcs/0005-json-family-production-v1.md) and the explicit character
  tables https://github.com/consema/consema-rs/blob/main/consema-json/src/parser.rs.

Unicode note (blind-write disclosure): RFC 0005 §4 pins Unicode 17.0.0
identifier tables (https://github.com/consema/consema/blob/main/docs/rfcs/0005-json-family-production-v1.md). This implementation classifies
...
class JsonProfile(enum.Enum):
    """Frozen JSON language profile (https://github.com/consema/consema-rs/blob/main/consema-json/src/lib.rs)."""
...
class JsonSyntaxKind(enum.Enum):
    ...
    (https://github.com/consema/consema-rs/blob/main/consema-json/src/lib.rs; the v1 kind set; Identifier is
...
class JsonValueKind(enum.Enum):
    """Native semantic value category (lib.rs and the mapping at
    lib.rs; BinaryFloat64 is the JSON5 extension, RFC 0005 §6)."""
...
class SemanticUnavailable(enum.Enum):
    """Regional reason why native semantics are unavailable (lib.rs)."""
...
# -- JSON5 character classes (parser.rs; RFC 0005 §3/§4) ------------

# Exact union table of parser.rs (RFC 0005 §3): U+0009 U+000A U+000B
...
def is_json5_whitespace(character: str) -> bool:
    """Exact union table of parser.rs (RFC 0005 §3)."""
