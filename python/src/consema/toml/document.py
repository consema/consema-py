...
- The temporal datum shape (TomlDate/TomlTime/TomlOffset/TomlDateTime)
  transcribes https://github.com/consema/consema-rs/blob/main/consema-toml/src/lib.rs.
...
class TomlProfile(enum.Enum):
    """Frozen TOML language profiles (https://github.com/consema/consema-rs/blob/main/consema-toml/src/lib.rs)."""
...
class TomlItemKind(enum.Enum):
    """Native TOML item category (https://github.com/consema/consema-rs/blob/main/consema-toml/src/lib.rs,
...
@dataclass(frozen=True, slots=True)
class TomlTime:
    """Parsed TOML time fields (lib.rs); the fractional second is
...
# ---------------------------------------------------------------------------
# Entity model (parser.rs)
# ---------------------------------------------------------------------------


@dataclass(frozen=True, slots=True)
class _InternalItemKind:
    """The closed internal item payload vocabulary (lib.rs)."""
...
class TomlAccessError(Exception):
    """Stable TOML native handle failure (lib.rs). Carries no
...
class TomlItem:
    ...
    def as_float_bits(self) -> int | None:
        """Exact IEEE-754 binary64 bit pattern (lib.rs)."""
        kind = self.document._item_entity(self.index).kind
...
class Document:
    ...
    def root(self) -> TomlItem:
        """Root native item, always a RootTable (lib.rs)."""
    ...
    def project(self, request: "object"):
        ...
        (projection.rs). See consema.toml.projection."""
