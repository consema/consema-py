...
_SINGLE_FROM = tuple(code for code, _ in _LOWER_SINGLES)
_SINGLE_TO = tuple(mapped for _, mapped in _LOWER_SINGLES)
...
def _simple_lowercase(code: int) -> int | None:
    ...
    for start, end, step, delta in _LOWER_RANGES:
        if start <= code <= end and (code - start) % step == 0:
            return code + delta
    for index, source in enumerate(_SINGLE_FROM):
        if source == code:
            return _SINGLE_TO[index]
    return None
